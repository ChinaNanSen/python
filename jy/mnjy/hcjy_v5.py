...
def backtest(symbol, start_date, end_date, timeframe, initial_balance):
    ...
    for index, row in data.iterrows():
        ...
        # exit(1)
        # 检查是否需要开多头仓位
        # if position == 0 and balance > 100  and row['ema30'] > row['ema10'] and \
        ...
        # if position > 0 and row['ema20'] > row['ema10']:
            ...
            totee += etee
            position = 0  # 重置持仓
            print(f"Date: {row['ts']}, Position Closed: {position}, Balance: {balance}, PnL: {position_pnl}, Price: {row['close']}, etee: {etee},  totald: {totald}")
